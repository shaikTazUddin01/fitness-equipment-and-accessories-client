    ...
    <div className="card bg-base-100  shadow-xl rounded-md hover:shadow-2xl">
      <a href={`/productDetails/${_id}`}>
        <figure className="px-2 pt-2 ">
          <img src={images} alt={name} className=" w-[95%] object-cover h-[150px]" />
        </figure>
      </a>
      <div className="card-body px-5">
        ...
        <span className="flex items-center gap-1 -mt-1">
          <Rate disabled allowHalf defaultValue={Number(aveRating)} className="custom-rate text-[13px]" />
          <p className="text-sm text-[#515151]">({reviews?.length ?reviews?.length: "0"})</p>
        </span>
        {
          pathname !=="/" &&
        <button
